...
public class LabRequestController {
    ...
    public List<TestRequest> getForTester()  {
        try
        {
            //Retrieving the current user/tester
            User user = userLoggedInService.getLoggedInUser();
            if(user != null)
            {
                //Retrieving and return the test requests assigned to the tester
        ...
        {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to find the test requests assigned to a tester");
        }
    }
